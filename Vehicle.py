...
class Vehicle():
    ...
    def draw(self, rect=False, velocity=False, acceleration=False, slowing_field=False, arrow=True):
        # self rect
        if rect:
        ...
        if velocity:
            pygame.draw.aaline(self.screen, self.colour, self.rect.center,
                               (self.rect.center[0] + self.vel.x * 10, self.rect.center[1] + self.vel.y * 10))

        # Acceleration line (steering force)
        if acceleration:
            pygame.draw.aaline(self.screen, (0, 255, 0), self.rect.center,
                               (self.rect.center[0] + self.acc.x * 150, self.rect.center[1] + self.acc.y * 150))

        # slowing field
        if slowing_field:
            pygame.draw.circle(self.screen, self.colour,
                               self.rect.center, int(self.slowing_distance), 1)

        # arrow
        if arrow:
            point1 = Vector2(self.vel)
            ...
            pygame.draw.aalines(self.screen, self.colour, True,
                                [point1 + self.pos, point2 + self.pos, self.pos, point3 + self.pos])
    ...
    def follow_path(self, path_object):
        ...
        draw_normal_future_pos = (int(pos_on_line_future.x),int(pos_on_line_future.y))
        draw_future_pos = (int(future_pos.x),int(future_pos.y))
        pygame.draw.aaline(self.screen, colour, draw_future_pos, draw_normal_pos)
